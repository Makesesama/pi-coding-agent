...
MELPA installs =transient= and =markdown-table-wrap= automatically.
=pi-coding-agent= uses Emacs 31's built-in =markdown-ts-mode= only for
its own chat and input buffers, so loading this package does not change
how unrelated =.md= files open.
